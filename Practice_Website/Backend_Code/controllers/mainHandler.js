...
/**
 * @description Method to get all of the records on the Admin table
 ...
 */
exports.getAdminRecordByAdminID = async (req, res) => {
    try{
        let [ sAdminId ] = req.params;
 ...
 * @returns A record consisting of 4 columns (Admin ID, First Name, Middle Name, Last Name)
 */
exports.getAdminRecordByName = async (req, res) => {
    try{
...
exports.addAdminRecord = async (req, res) => {
    try{
        ...
        if (supabaseError)
            throw supabaseError;
        res.status(201).json({ success : true, message : 'Admin record added successfully', data : data[0]})
    } catch (generalError) {
        handleSupabaseError(res, error);
    }
}
...
exports.updateAdminRecord = async (req, res) => {
    try{
        let { oldAdminId } = req.params
        let { newAdminId, fName, mName, lName, oldAdminPassword, newAdminPassword } = req.query
        ...
        const {data, error: supabaseError} = await supabaseClient.rpc('update_admin_record', {
            ...
            input_new_admin_password : newAdminPassword
        })
        
        if (supabaseError)
            throw supabaseError
        res.status(200).json({ status : true, message : 'Admin updated successfully', data : data[0]})
    } catch (generalError) {
        handleSupabaseError(res, error)
    }
}
