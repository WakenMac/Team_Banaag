...
addGlasswareForm.addEventListener("submit", async (e) => {
  ...
  let result = await addGlasswaresRecord(
    glasswareName,
    glasswareLocation,
    glasswareUnit,
    glasswareBrand,
...
editGlasswareForm.addEventListener("submit", async function (e) {
  ...
  let result = await updateGlasswaresRecordByAll(
    editGlasswareId,
    editGlasswareName,
...
confirmDeleteGlasswareBtn.addEventListener("click", async () => {
  ...
  const glasswareId = glasswareRowToDelete.children[0].textContent.trim();

  let result = await deleteGlasswaresRecordByItemId(glasswareId);
  result = (result != null)? result.data : null;
...
remarksForm.addEventListener("submit", async (e) => {
  ...
  let result = await updateGlasswaresRemarkByItemId(
    glasswareId,
    remarks
  );
  result = (result != null)? result.data : null;
...
async function prepareGlasswaresTable() {
  try {
    let data = await getAllGlasswaresRecords();
    glasswareData = data = data.data; // store for PDF use
    if (data.length == 0) {
...
async function getAllGlasswaresRecords(){
  try {
    let response = await fetch('/glasswares/get-all-glasswares-records', {
      method: 'GET'
    }); 
...
async function addGlasswaresRecord(
  glasswareName,
  glasswareLocation,
  glasswareUnit,
  glasswareBrand,
  ...
  try {
    let response = await fetch('/glasswares/add-glasswares-records', {
      method: 'POST',
      headers : {
        'Content-type' : 'application/json'
      },
      body : JSON.stringify({
        itemName : glasswareName, 
        locationName : glasswareLocation, 
        unitTypeName : glasswareUnit, 
        brandModel : glasswareBrand, 
        remarks : remarks
      })
    }); 
...
async function updateGlasswaresRecordByAll(
  editGlasswareId,
  editGlasswareName,
  editGlasswareLocation,
  editGlasswareUnit,
  ...
  try {
    let response = await fetch('/glasswares/update-glasswares-records', {
      method: 'PATCH',
      headers : {
        'Content-type' : 'application/json'
      },
      body : JSON.stringify({
        itemId : editGlasswareId,
        itemName : editGlasswareName, 
        locationName : editGlasswareLocation, 
        unitTypeName : editGlasswareUnit, 
        brandModel : editGlasswareBrand, 
        remarks : remarks
      })
    }); 
...
async function updateGlasswaresRemarkByItemId(glasswareId, remarks){
  try {
    let response = await fetch('/glasswares/update-glasswares-remarks', {
      method: 'PATCH',
      headers : {
        'Content-type' : 'application/json'
      },
...
async function deleteGlasswaresRecordByItemId(glasswareId){
  try {
    let response = await fetch('/glasswares/delete-glasswares-records', {
      method: 'DELETE',
      headers : {
        'Content-type' : 'application/json'
      },
