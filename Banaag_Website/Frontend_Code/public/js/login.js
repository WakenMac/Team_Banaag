...
async function validateSignIn(event) {
  ...
  let result = await addAdminRecord(adminId, firstName, middleName, lastName, password)
  showOverlay();

  console.log(result);

  if (!result.data || result.data.includes("ERROR")) {
...
async function validateLogin(event) {
  ...
  let result = await adminExists(adminId, password);
  showOverlay();
  console.log(result);
  ...
  if (result.exists === true) {
    ...
    // Failed Login
    showNotification("Invalid User Credentials", 'false')
    showOverlay(false);
  }
}
...
async function addAdminRecord(adminId, firstName, middleName, lastName, password){
  try{
    const response = await fetch(`/admins/add-admin-record`, {
      method: 'PUT',
      headers: {
        'Content-type' : 'application/json'
      },
