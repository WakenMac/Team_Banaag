...
addUnitTypeForm.addEventListener("submit", async (e) => {
  ...
  let result = await addUnitTypeRecord(unitTypeName);
...
editUnitTypeForm.addEventListener("submit", async (e) => {
  ...
  let result = await updateUnitTypeRecordName(
    editUnitTypeId,
    editUnitTypeName
  );
  result = (result != null)? result.data : null;
...
confirmDeleteBtn.addEventListener("click", async () => {
  if (unitTypeToDelete && rowToDelete) {
    let result = await removeUnitTypeRecordByUnitTypeId(
      unitTypeToDelete
    );
    result = (result != null)? result.data : null;
    if (result == null) {
...
async function prepareUnitTypeTable() {
  try {
    let data = await getAllUnitTypeRecordsOrderById();
    data = data.data;
    if (data.length == 0) {
...
async function getAllUnitTypeRecordsOrderById(){
  try {
    let response = await fetch('/unit-type/get-all-unit-type-order-by-id', {
      method: 'GET',
    }); 
...
async function addUnitTypeRecord(unitTypeName){
  try {
    let response = await fetch('/unit-type/add-unit-type-record', {
      method: 'POST',
      headers : {
...
async function updateUnitTypeRecordName(editUnitTypeId, editUnitTypeName){
    try {
    let response = await fetch('/unit-type/update-unit-type-record', {
      method: 'PATCH',
      headers : {
        'Content-type' : 'application/json'
      },
...
async function removeUnitTypeRecordByUnitTypeId(unitTypeId){
    try {
    let response = await fetch('/unit-type/remove-unit-type-record', {
      method: 'DELETE',
      headers : {
        'Content-type' : 'application/json'
      },
