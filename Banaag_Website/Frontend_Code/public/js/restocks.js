...
function setupEventListeners() {
  ...
  addRestockItemName.addEventListener('input', () => {
    ...
    if (searchIndex != -1){
      ...
      if (referenceTable[searchIndex]["Item Type"] !== 'Chemicals' &&
          referenceTable[searchIndex]["Item Type"] !== 'Consumable Items')
        addRestockForm.restockExpirationDate.disabled = true;
      else 
        addRestockForm.restockExpirationDate.disabled = false;
  ...
  if (remarksForm) {
    remarksForm.addEventListener("submit", async (e) => {
      ...
      let result = await updateRestocksRemarksByRestockId(restockId, remarks);
...
function createNewRestockRow(
  ...
  if (restockExpirationDate === 'null' || restockExpirationDate == null)
    restockExpirationDate = 'Non-Expiry'
...
async function handleAddRestockSubmit(e) {
  ...
  const isDisabled = addRestockForm.restockExpirationDate.disabled;
  ...
  if (
    ...
    !restockBrand ||
    !restockDate ||
    (!restockExpirationDate && isDisabled == false) 
  ) {
  ...
  let result = await addRestocksRecord(
    itemId,
    restockDate,
    restockExpirationDate,
    restockQuantity,
...
// EDITING
async function handleEditRestockSubmit(e) {
  e.preventDefault();
  ...
  var restockExpirationDate = document.getElementById(
    "editRestockExpirationDate"
  ).value;
  const isDisabled = editRestockForm.editRestockExpirationDate.disabled
  ...
  // let remarks = document.querySelector(`button[data-restock-id="${restockId}"]`).getAttribute('data-remarks')
  ...
  updateRestockTable(
    restockId,
    restockItemName,
    document.getElementById("editRestockQuantity").value.trim(),
    document.getElementById("editRestockBrand").value.trim(),
    restockDate,
    restockExpirationDate
  );
...
// DELETE (Not included in the program)
...
function populateEditForm(row) {
  ...
  let itemType = referenceTable.find(item => item["Item ID"] === row.children[0].textContent.trim())
  if (itemType !== 'Chemicals' || itemType !== 'Consumable Items'){
    editRestockForm.editRestockExpirationDate.disabled = true;
    editRestockForm.editRestockQuantity.disabled = false;
  } else{
    editRestockForm.editRestockExpirationDate.disabled = false;
    editRestockForm.editRestockQuantity.disabled = true;
  } 
...
async function prepareItemData() {
  try {
    showLoading();
    let data = await getAllItemMasterListRecords();
    itemData = data.data;
  } catch (generalError) {
...
async function initializeRestocksTable() {
  try {
    showLoading();
    let data = await getAllRestocksRecords();
    data = data.data;
    if (data.length == 0) {
...
async function initializeDataList(){
  try {
    let data = await getAllItemMasterListNameRecords();
    data = data.data
    if (data.length == 0) {
...
async function initializeReferenceTable(){
  try {
    let data = await getAllItemMasterListNameBrandRecords();
    data = data.data
    if (data.length == 0) {
...
// Loading Spinner and Table Loading State Functions
function showLoading() {
  const loadingSpinner = document.getElementById('loadingSpinner');
  const tableLoadingState = document.getElementById('tableLoadingState');
  if (loadingSpinner) loadingSpinner.style.display = 'flex';
  if (tableLoadingState) tableLoadingState.style.display = 'block';
}
function hideLoading() {
  const loadingSpinner = document.getElementById('loadingSpinner');
  const tableLoadingState = document.getElementById('tableLoadingState');
  if (loadingSpinner) loadingSpinner.style.display = 'none';
  if (tableLoadingState) tableLoadingState.style.display = 'none';
}
function showTableLoading() {
  const table = document.getElementById('restocksTableBody');
  if (!table) return;
  table.innerHTML = `
...
async function getAllItemMasterListRecords(){
  try {
    let response = await fetch('/iml/get-all-iml-records', {
      method: 'GET'
    }); 
...
async function getAllRestocksRecords(){
    try {
    let response = await fetch('/restocks/get-all-restocks-records', {
      method: 'GET',
    }); 
...
async function getAllItemMasterListNameRecords(){
    try {
    let response = await fetch('/iml/get-all-iml-name-records', {
      method: 'GET',
    }); 
...
async function getAllItemMasterListNameBrandRecords(){
    try {
    let response = await fetch('/iml/get-all-iml-name-brand-records', {
      method: 'GET',
    }); 
...
async function addRestocksRecord(
  itemId,
  restockDate,
  restockExpirationDate,
  restockQuantity,
  remarks
    ...
    try {
    let response = await fetch('/restocks/add-restocks-records', {
      method: 'POST',
      headers : {
      ...
      body : JSON.stringify({
        ...
        expiryDate : restockExpirationDate, 
...
async function updateRestocksRemarksByRestockId(restockId, remarks){
    try {
    let response = await fetch('/restocks/update-restocks-remarks', {
      method: 'PATCH',
      headers : {
        'Content-type' : 'application/json'
      },
